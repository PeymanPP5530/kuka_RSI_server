...
def calculate_corrections():
    ...
    # When first connecting, set target to current position if not already set
    if packet_counter < 10:  # During the first few packets
        for axis in target_positions.keys():
            if target_positions[axis] == 0.0:  # Assuming 0.0 means not set
                target_positions[axis] = current_positions[axis]
                print(f"Initializing target {axis} to current position: {current_positions[axis]:.2f}")
                logging.info(f"Initializing target {axis} to current position: {current_positions[axis]:.2f}")
...
def user_input_thread():
    ...
    while running:
        ...
        if cmd.lower() == "exit":
            ...
            global connected, packet_counter, last_packet_time, current_step
        ...
        while running:
            try:
                ...
                try:
                    ...
                    # Extract Step value (current waypoint)
                    step_elem = root.find(".//Step")
                    if step_elem is not None and step_elem.text:
                        new_step = int(step_elem.text)
                        if new_step != current_step:
                            print(f"Step changed: {current_step} → {new_step}")
                            print(f"{current_step}")
                            logging.info(f"Step changed: {current_step} → {new_step}")
...
def main():
    ...
    print("Using XML config with 'Step' for waypoint tracking")
