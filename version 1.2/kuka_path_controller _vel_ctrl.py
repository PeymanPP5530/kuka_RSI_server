...
ramp_up_factor = 0.001  # Initial ramp-up factor (starts small)
...
# for corrections like (correction X 100) the correction values ar in mm


def correction_send():
    """Calculate corrections using proportional control for smoother movement."""
    global correction_val, initial_corr_pos, current_positions, target_positions, ramp_up_factor


    # to check if the corrected values are stored correctly
    print("Current correction values:", correction_val)

    # Control parameters
    position_gain = 0.005      # Gain for position corrections
    rotation_gain = 0.005       # Gain for rotation corrections
    max_correction_position = 0.7  # Maximum position correction (mm per cycle)
    max_correction_angle = 0.0025   # Maximum rotation correction (degrees per cycle)
    ramp_up_increment = 0.001  # Increment for ramp-up factor per cycle
    max_ramp_up = 1.0  # Maximum ramp-up factor

    # Initialize corrections dictionary once , the values from command are stored in 
    corrections = {axis: 0.0 for axis in ["X", "Y", "Z", "A", "B", "C"]}
    
    # Apply corrections for all axes that have non-zero correction values correction_val which is global and retains values
    for axis in ["X", "Y", "Z", "A", "B", "C"]:
        if correction_val[axis] != 0.0:
            if axis in ["A", "B", "C"]:
                # Handle rotational axes
                ...
                if corrections[axis] < 0.001 and corrections[axis] > -0.001:
                    correction_val[axis] = 0.0
            else:
                # Handle linear axes (X, Y, Z)
                target_positions[axis] = correction_val[axis] + initial_corr_pos[axis]
                diff = target_positions[axis] - current_positions[axis]
                corrections[axis] = max(-max_correction_position, min(max_correction_position, position_gain * diff))
                if corrections[axis] < 0.0001 and corrections[axis] > -0.0001:
                    correction_val[axis] = 0.0


            #  Apply ramp-up factor to smooth the start
            corrections[axis] *= ramp_up_factor

    # Gradually increase the ramp-up factor until it reaches the maximum
    if ramp_up_factor < max_ramp_up:
        ramp_up_factor = min(max_ramp_up, ramp_up_factor + ramp_up_increment)
...
def user_input_thread():
    ...
    global running, target_positions, target_mode, current_positions, initial_corr_pos, stop_flag, routine_num, routine_user_flag, routine_user, ramp_up_factor
    ...
    while running:
        ...
        if cmd.lower() == "exit":
            ...
            print("  correction X 50 - Set the correction value for an axis directly")
        ...
        # Parse format like (correction X 50)  , controlled using target_mode= FALSE     
    
        elif cmd.lower().startswith("correction"):
            parts = cmd.split()
            if len(parts) == 3 and parts[1].upper() in target_positions:
                try:
                    # Check if this is the first correction command
                    # if all(correction_val[axis] == 0.0 for axis in correction_val):
                        # If all corrections are currently zero, initialize positions
                    for axis in ["X", "Y", "Z", "A", "B", "C"]:
                        initial_corr_pos[axis] = current_positions[axis]
                    
                    ramp_up_factor = 0.001  # Reset ramp-up factor for new corrections
                    target_mode = False
                    axis = parts[1].upper()
                    value = float(parts[2])
                    correction_val[axis] = value  # Just set this axis without zeroing others
                ...
                print("Error: Command format should be 'correction AXIS VALUE'")
    ...
    try:
        while running:
            try:
                ...
                if target_mode :
                    corrections = calculate_corrections() # for targets like (X,Y,Z,A,B,C)
                else:   
                    corrections = correction_send()     # for corrections like (correction X 100) 
