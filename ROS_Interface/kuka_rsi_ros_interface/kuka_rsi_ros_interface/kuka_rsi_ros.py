...
class KukaRsiNode(Node):
    ...
    def routine_callback(self, msg: Int32):
        """Callback for receiving routine commands."""
        self.get_logger().info(f"Received routine command: {msg.data}")
        with self.comm_lock:
            if isinstance(msg.data, int) and msg.data >= 0: # Basic validation
                 self.routine_user.append(msg.data)
                 self.get_logger().info(f"Added routine {msg.data} to queue (queue size: {len(self.routine_user)}).")
            else:
                 self.get_logger().warn(f"Invalid routine command received: {msg.data}")
